    ...
    <div class="app-container">
        ...
        <header class="topbar">
            <div class="topbar-left">
                <a href="{{ route('categorias') }}" class="back-btn">
        ...
        <div class="profile-grid">
            ...
            <div class="profile-card">
                <div class="avatar-wrap" onclick="openModal('avatarModal')" title="Haz clic para cambiar tu foto o avatar">
                    <div class="avatar-frame">
                        <div class="avatar-img-inner" id="currentAvatarDisplay">
                            @if($user->avatar && (str_starts_with($user->avatar, '/uploads/') || str_starts_with($user->avatar, 'http')))
                                <img src="{{ asset($user->avatar) }}" alt="Avatar de {{ $user->name }}">
                            @elseif($user->avatar === 'llama')
                                <span style="font-size: 60px;">🦙</span>
                            @elseif($user->avatar === 'condor')
                                <span style="font-size: 60px;">🦅</span>
                            @elseif($user->avatar === 'inca')
                                <span style="font-size: 60px;">👑</span>
                            @elseif($user->avatar === 'coya')
                                <span style="font-size: 60px;">👸</span>
                            @elseif($user->avatar === 'inti')
                                <span style="font-size: 60px;">☀️</span>
                            @elseif($user->avatar === 'chakana')
                                <span style="font-size: 60px;">🏔️</span>
                            @elseif($user->avatar === 'puma')
                                <span style="font-size: 60px;">🏹</span>
                            @elseif($user->avatar === 'diablada')
                                <span style="font-size: 60px;">🎭</span>
                            @else
                ...
                <div class="league-badge">
                    <span>{{ $liga['icono'] }}</span>
                    <span>{{ $liga['nombre'] }}</span>
                </div>

                <div class="user-joined">
                    🗓️ Miembro desde {{ $user->created_at ? $user->created_at->translatedFormat('F Y') : 'Reciente' }}
                </div>
            ...
            <div class="content-col">
                ...
                <div class="streak-card">
                        ...
                        <div class="streak-title-wrap">
                            ...
                            <div>
                                <div class="streak-count">
                                    <span>{{ $racha }}</span> {{ $racha === 1 ? 'DÍA DE RACHA' : 'DÍAS DE RACHA' }}
                                </div>
                    ...
                    <div class="streak-week">
                        @foreach($calendarioRacha as $dia)
                            <div class="week-day {{ $dia['activo'] ? 'active' : '' }} {{ $dia['es_hoy'] ? 'today' : '' }}">
                                <span class="day-letter">{{ $dia['letra'] }}</span>
                                <div class="day-bubble" title="{{ $dia['nombre'] }}{{ $dia['es_hoy'] ? ' (Hoy)' : '' }}">
                                    @if($dia['activo'])
                                        🔥
                                    @elseif($dia['es_hoy'])
                                        ⚡
                ...
                <div class="stats-card">
                    ...
                    <div class="level-progress-box">
                        <div class="level-head">
                            <div class="level-name">
                                ...
                                <span>Nivel {{ $nivelActual }} · {{ $nivelInfo['nombre'] }}</span>
                            </div>
                            <div class="level-xp-text">
                                {{ $xpEnNivel }} / {{ $xpRequerida }} XP
                            </div>
                        </div>
                        <div class="xp-bar-track">
                            <div class="xp-bar-fill" style="width: {{ $progresoNivel }}%;"></div>
                        </div>
                    </div>
                    ...
                    <div class="metrics-grid">
                        <div class="metric-box">
                            ...
                            <div class="metric-val">{{ $puntuacion }}</div>
                            <div class="metric-lbl">Puntos XP Totales</div>
                        </div>
                        <div class="metric-box">
                            <div class="metric-icon">💖</div>
                            <div class="metric-val">{{ $vidas }}/5</div>
                            <div class="metric-lbl">Vidas Disponibles</div>
                        </div>
                        <div class="metric-box">
                            <div class="metric-icon">📖</div>
                            <div class="metric-val">{{ $totalPalabras }}</div>
                            <div class="metric-lbl">Palabras Quechua</div>
                        </div>
                        <div class="metric-box">
                            <div class="metric-icon">🏔️</div>
                            <div class="metric-val">{{ $totalCategorias }}</div>
                ...
                <div class="stats-card">
                    <div class="section-header">
                        ...
                        <span style="font-size: 0.9em; color: var(--gold); font-weight: 700;">
                            {{ count(array_filter($logros, fn($l) => $l['completado'])) }} / {{ count($logros) }} Desbloqueados
                        </span>
                    </div>

                    <div class="achievements-grid">
                        @foreach($logros as $logro)
                            <div class="achievement-card {{ $logro['completado'] ? 'completed' : '' }}">
                                <div class="ach-icon-box">
                                    {{ $logro['icono'] }}
                                </div>
                                <div class="ach-info">
                                    <div class="ach-title">{{ $logro['titulo'] }}</div>
                                    <div class="ach-desc">{{ $logro['descripcion'] }}</div>
                                    <div class="ach-progress-wrap">
                                        <div class="ach-bar-track">
                                            <div class="ach-bar-fill" style="width: {{ ($logro['actual'] / max(1, $logro['maximo'])) * 100 }}%; background: {{ $logro['color'] }};"></div>
                                        </div>
                                        <span class="ach-badge-status {{ $logro['completado'] ? '' : 'locked' }}">
                                            {{ $logro['completado'] ? 'COMPLETADO ✨' : $logro['actual'].'/'.$logro['maximo'] }}
                                        </span>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="modal-overlay" id="avatarModal">
        <div class="modal-box">
            ...
            <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" id="avatarForm">
                ...
                <input type="hidden" name="avatar_preset" id="selectedPresetInput" value="{{ $user->avatar }}">
                ...
                <div id="presetsTab">
                    <div class="avatar-presets-grid">
                        <div class="preset-item {{ $user->avatar === 'llama' ? 'selected' : '' }}" onclick="selectPreset('llama', this)">
                            <div class="preset-icon">🦙</div>
                            <div class="preset-name">Llama</div>
                        </div>
                        <div class="preset-item {{ $user->avatar === 'condor' ? 'selected' : '' }}" onclick="selectPreset('condor', this)">
                            <div class="preset-icon">🦅</div>
                            <div class="preset-name">Cóndor</div>
                        </div>
                        <div class="preset-item {{ $user->avatar === 'inca' ? 'selected' : '' }}" onclick="selectPreset('inca', this)">
                            <div class="preset-icon">👑</div>
                            <div class="preset-name">Inca</div>
                        </div>
                        <div class="preset-item {{ $user->avatar === 'coya' ? 'selected' : '' }}" onclick="selectPreset('coya', this)">
                            <div class="preset-icon">👸</div>
                            <div class="preset-name">Coya</div>
                        </div>
                        <div class="preset-item {{ $user->avatar === 'inti' ? 'selected' : '' }}" onclick="selectPreset('inti', this)">
                            <div class="preset-icon">☀️</div>
                            <div class="preset-name">Inti</div>
                        </div>
                        <div class="preset-item {{ $user->avatar === 'chakana' ? 'selected' : '' }}" onclick="selectPreset('chakana', this)">
                            <div class="preset-icon">🏔️</div>
                            <div class="preset-name">Chakana</div>
                        </div>
                        <div class="preset-item {{ $user->avatar === 'puma' ? 'selected' : '' }}" onclick="selectPreset('puma', this)">
                            <div class="preset-icon">🏹</div>
                            <div class="preset-name">Guerrero</div>
                        </div>
                        <div class="preset-item {{ $user->avatar === 'diablada' ? 'selected' : '' }}" onclick="selectPreset('diablada', this)">
                            <div class="preset-icon">🎭</div>
        ...
        <div class="modal-box">
            ...
            <form method="POST" action="{{ route('profile.update') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="editName" name="name" class="form-input" value="{{ old('name', $user->name) }}" required>
                </div>

                <div class="form-group">
                    <label class="form-label" for="editEmail">Correo Electrónico</label>
                    <input type="email" id="editEmail" name="email" class="form-input" value="{{ old('email', $user->email) }}" required>
                </div>
